...
def tenant(PROJECT_NAME, NETWORK_NAME, SUBNET_NAME, VM_NAME):
    ...
    # CIDR 추출
    conn, curs = db_sql_tenant.conn_db()
    sql_CIDR = f"SELECT CIDR FROM SUBNET WHERE NAME='{SUBNET_NAME}'"
    result_sql_CIDR = curs.execute(sql_CIDR).fetchall()
    CIDR = str(result_sql_CIDR[0][0])
    conn.close()
    ...
    # 보안그룹 인바운드 룰 적용 (bvt)
    UpdateSecurityGroupRuleService = {
        "header": {
            "targetServiceName": "network/com.tmax.tmaxcloud.network.master.securitygroup.UpdateSecurityGroupRuleService",
            "messageType": "REQUEST",
            "contentType": "TEXT"
        },
        "body": {
    ...
    file_path, total_num_files, base_direc = create_json_file("UpdateSecurityGroupRuleService.json", UpdateSecurityGroupRuleService, total_num_files)
    success_num_files, fail_num_files = sapgo_result.tenant_sapgo_result(base_direc, file_path, success_num_files, fail_num_files)
    db_sql_tenant.result_tenant_sql()
    ...
    BindNaclService = {
        "header": {
            "targetServiceName": "network/com.tmax.tmaxcloud.network.master.nacl.BindNaclService",
            "messageType": "REQUEST"
        },
        "body": {
            "tenant_id" : db_sql_tenant.result_TENANT_ID,
    ...
    file_path, total_num_files, base_direc = create_json_file("BindNaclService.json", BindNaclService, total_num_files)
    success_num_files, fail_num_files = sapgo_result.tenant_sapgo_result(base_direc, file_path, success_num_files, fail_num_files)
    db_sql_tenant.result_tenant_sql()
    ...
    # 1번 GUEST_MACHINE_ID 추출 (VM_NAME)
    conn, curs = db_sql_tenant.conn_db()
    sql_FIR_GUEST_MACHINE_ID = f"SELECT GUEST_MACHINE_ID FROM GUEST_MACHINE WHERE NAME = '{VM_NAME}'"
    result_sql_FIR_GUEST_MACHINE_ID = curs.execute(sql_FIR_GUEST_MACHINE_ID).fetchall()
    FIR_GUEST_MACHINE_ID = int(result_sql_FIR_GUEST_MACHINE_ID[0][0])
    conn.close()
    ...
    RunGuestMachineService = {
        ...
        "body": {
            "guest_machine_id" : FIR_GUEST_MACHINE_ID,
            "project_id" : PROJECT_ID
        }
    }
    ...
    # 2번 GUEST_MACHINE_ID 추출 (가장 최근에 생성된 가상머신)
    conn, curs = db_sql_tenant.conn_db()
    sql_SEC_GUEST_MACHINE_ID = f"SELECT GUEST_MACHINE_ID FROM GUEST_MACHINE WHERE CREATED_TIME = (SELECT MAX(CREATED_TIME) FROM GUEST_MACHINE)"
    result_sql_SEC_GUEST_MACHINE_ID = curs.execute(sql_SEC_GUEST_MACHINE_ID).fetchall()
    SEC_GUEST_MACHINE_ID = int(result_sql_SEC_GUEST_MACHINE_ID[0][0])
    conn.close()
    ...
    RunGuestMachineService = {
        ...
        "body": {
            "guest_machine_id" : SEC_GUEST_MACHINE_ID,
            "project_id" : PROJECT_ID
        }
    }
    ...
    CreateTargetGroupService  = {
        ...
        "body": {
            ...
            "create_members": [
                {
                    "guest_interface_id": FIR_GUEST_MACHINE_ID,
                    "target_ip": None,
                    "target_port": "80",
                    "rule_option": None
                },
                {
                    "guest_interface_id": SEC_GUEST_MACHINE_ID,
                    "target_ip": None,
                    "target_port": "80",
                    "rule_option": "11"
                }
    ...
    # FIP_ID 추출
    conn, curs = db_sql_tenant.conn_db()
    sql_FIP_ID = f"SELECT FLOATING_IP_ID FROM FLOATING_IP WHERE NETWORK_ID = '{NETWORK_ID}'"
    result_sql_FIP_ID = curs.execute(sql_FIP_ID).fetchall()
    FIP_ID = int(result_sql_FIP_ID[0][0])
    conn.close()

    # FIP 바인딩 (instance)
    BindFloatingIpService = {
        "header": {
            "targetServiceName": "network/com.tmax.tmaxcloud.network.master.floatingip.BindFloatingIpService",
            "messageType": "REQUEST"
        },
        "body": {
            "tenant_id": db_sql_tenant.result_TENANT_ID,
            ...
            "resource_id": FIR_GUEST_MACHINE_ID,
            "resource_name": "Auto_bind_vm_fip",
            "resource_ip": None
        }
    }
    file_path, total_num_files, base_direc = create_json_file("BindFloatingIpService.json", BindFloatingIpService, total_num_files)
    success_num_files, fail_num_files = sapgo_result.tenant_sapgo_result(base_direc, file_path, success_num_files, fail_num_files)
    db_sql_tenant.result_tenant_sql()
...
if __name__ == "__main__":
    ...
    tenant(PROJECT_NAME, NETWORK_NAME, SUBNET_NAME, VM_NAME)
